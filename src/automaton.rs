...
pub fn main<'d: 'a, 'a>(
	// rawgrammar:Vec<Vec<&'a str>>,
	rules: &'a Vec<Rule<'a>>,
	rulelist: &'a RuleList<'a>,
	firsts: &'d HashMap<&'a str, HashSet<&'a str>>,
) -> Option<((u64, Vec<(u64, RuleItems<'a>)>), AutomatonTable<'a>)> {
	// let rules = from_rules_literal(from_raw_rules(rawgrammar));
	...
	if let Some(initial) = RuleItems::initial(rulelist) {
		...
		// let ((_, hashpair), table) =
		Some(RuleItems::derivate(
			initial.clone(),
			&random,
			&firsts,
			rulelist,
...
impl<'a> Token<'a> {
	...
	pub fn to_ahead(self) -> Ahead<'a> {
		Ahead::S(self.to_str())
	}
}

#[derive(Debug, Clone)]
pub struct RuleLiteral<'a> {
	left: String,
	right: Vec<&'a str>,
}
...
pub fn from_raw_rules<'a>(rules: Vec<Vec<&'a str>>) -> Vec<RuleLiteral<'a>> {
	rules
		.into_iter()
...
pub struct RuleList<'a>(pub &'a [Rule<'a>]);

impl<'a> RuleList<'a> {
	pub fn at(&self, i: &usize) -> Option<&'a Rule<'a>> {
		self.0.into_iter().find(|r| r.id == *i)
	}

	pub fn nonterminals(&self) -> HashSet<&str> {
	...
	pub fn terminals(&self) -> HashSet<&str> {
		...
		for rule in self.0.into_iter() {
			hs.extend(rule.right.iter().filter_map(|token| match token {
				Token::Terminal(s) => Some(s),
				_ => None,
			}));
		}
		hs
...
pub struct RuleItem<'a> {
	pub rule: &'a Rule<'a>,
	pub cursor: usize,
	pub ahead: HashSet<Ahead<'a>>,
}

#[derive(Debug, Clone, Hash)]
pub struct RuleItems<'a>(pub Vec<RuleItem<'a>>);
...
impl<'a, 'd> RuleItem<'a> {
	pub fn atcursor(&self) -> Option<&'d Token<'a>> {
		self.rule.right.get(self.cursor)
	}

	pub fn atcursorend(&self) -> Option<&'d Token<'a>> {
		self.rule.right.get(self.cursor)
	}
...
pub type AutomatonRow<'a> = HashMap<Tokenkey<'a>, u64>;
pub type AutomatonTable<'a> = HashMap<u64, AutomatonRow<'a>>;

impl<'a> Token<'a> {
	fn to_tokenkey(self) -> Tokenkey<'a> {
		match self {
			Self::Nonterminal(s) => Tokenkey::Nonterminal(s),
			Self::Terminal(s) => Tokenkey::Terminal(Ahead::S(s)),
		}
	}
}
...
pub fn from_rules_literal(rules: Vec<RuleLiteral>) -> Vec<Rule> {
	// let mut rs = Vec::new();
...
pub fn all_first<'a>(rules: &'a [Rule]) -> HashMap<&'a str, HashSet<&'a str>> {
	let h = HashMap::new();
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Tabled)]
pub enum Ahead<'a> {
	End,
	S(&'a str),
}
...
impl<'a> Ahead<'a> {
	fn as_bytes(self) -> &'a [u8] {
		match self {
			Ahead::End => "$$$$$$$$$$$$$$$$$$$$$$$".as_bytes(),
			Ahead::S(s) => s.as_bytes(),
		}
	}
}

impl ToString for Ahead<'_> {
	fn to_string(&self) -> String {
		match self {
			Ahead::End => "$End".to_string(),
			Ahead::S(s) => s.to_string(),
		}
	}
}

impl<'a> From<&'a str> for Ahead<'a> {
		...
		Some(RuleItems(
			r.into_iter()
				.map(|r| RuleItem {
					rule: r,
					cursor: 0,
					ahead: HashSet::from_iter([Ahead::End]),
				})
				.collect(),
		))
	}
			...
			.filter_map(|(selfi, item)| match item.rule.right.get(item.cursor) {
				Some(Token::Nonterminal(nonterminal)) => {
					let ah = match item.rule.right.get(item.cursor + 1) {
						Some(Token::Terminal(terminal)) => Some(HashSet::from([Ahead::S(*terminal)])),
						Some(Token::Nonterminal(non)) => {
							Some(firsts.get(non)?.iter().map(|s| Ahead::S(*s)).collect())
						}
						_ => None,
					};
