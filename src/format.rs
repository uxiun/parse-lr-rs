...
use crate::{
	...
	show, showshort,
	table::{
		NonterminalTable, NonterminalTableRow, TerminalOpe, TerminalTable, TerminalTableRow, TT,
	},
};
...
where
	...
	N: ToString, // N: ToString + Display,
{
	// let (iddict, tt) = simplify_terminal_table(tt);
	let mut terminals: Vec<_> = rulelist
		.terminals()
		.into_iter()
		.map(|s| s.to_string())
		.collect();
	terminals.push(Ahead::End.to_string());
	let mut nonterminals: Vec<_> = rulelist
		.nonterminals()
		.into_iter()
		.map(|s| s.to_string())
		.collect();
	...
	let tokens = [terminals.clone(), nonterminals.clone()];
	...
	// 	))
	// )
	let ntstr: HashMap<_, HashMap<String, String>> = nt
		.into_iter()
		.map(|(h, row)| {
			(
	...
	N: Clone,
{
	...
	let get_iddict = |key| *dict.get(key).unwrap_or(&(10000 as usize));
